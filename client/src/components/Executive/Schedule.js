...
import { Button } from "@/components/ui/button";
import { Tabs, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { format, addDays, isSameDay, isSameMonth, startOfMonth, startOfWeek, isBefore, startOfDay, } from "date-fns";
import { Calendar as CalendarIcon, Plus, ChevronLeft, ChevronRight, Grid, Sun, Moon, } from "lucide-react";
import EventModal from "./EventModal";
/* helpers (unchanged) */
function generateMonthGrid(currentDate) {
    const monthStart = startOfMonth(currentDate);
...
function SlotModal({ open, onClose, slot, events = [], onCreate, theme }) {
    ...
    const dateKey = format(slot.date, "yyyy-MM-dd");
    const isDark = theme === "dark";
    return (_jsxs("div", { className: "fixed inset-0 z-[20000] flex items-center justify-center p-4", children: [_jsx("div", { className: "absolute inset-0 bg-black/40", onClick: onClose }), _jsxs("div", { className: `relative w-full max-w-md rounded-xl p-5 shadow-2xl transform transition-all ${isDark ? "bg-slate-900 text-slate-100" : "bg-white text-slate-900"}`, onClick: (e) => e.stopPropagation(), children: [_jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { children: [_jsx("div", { className: "text-sm font-semibold", children: "Selected slot" }), _jsxs("div", { className: "text-lg font-bold", children: [dateKey, " \u00B7 ", _jsx("span", { className: "font-medium", children: slot.start })] }), _jsx("div", { className: "text-xs text-muted-foreground mt-1", children: "Create a focused 1-hour meeting at this time." })] }), _jsx("button", { "aria-label": "Close", className: "text-muted-foreground hover:text-gray-400", onClick: onClose, children: "\u2715" })] }), _jsx("div", { className: "mt-4", children: events.length === 0 ? (_jsx("div", { className: "text-sm text-muted-foreground", children: "No events at this slot." })) : (_jsx("div", { className: "space-y-2", children: events.map((ev) => (_jsxs("div", { className: `p-2 rounded-md border flex items-center justify-between ${isDark ? "border-slate-700" : "border-slate-200"}`, children: [_jsxs("div", { children: [_jsx("div", { className: "text-sm font-medium", children: ev.title }), _jsxs("div", { className: "text-xs text-muted-foreground", children: [ev.start, " \u2014 ", ev.end, " \u00B7 ", ev.venue || ""] })] }), _jsx("div", { className: `w-3 h-3 rounded-full ${ev.color}` })] }, ev.id))) })) }), _jsxs("div", { className: "mt-6 flex justify-end gap-3", children: [_jsx(Button, { variant: "ghost", onClick: onClose, children: "Close" }), _jsx(Button, { onClick: () => onCreate && onCreate(slot.date, slot.start), children: "Create" })] })] })] }));
}
/* SchedulePage */
export default function SchedulePage() {
    const themeContext = useContext(ThemeContext) || {};
    ...
    const [localTheme, setLocalTheme] = useState(isDarkFromContext ? "dark" : "light");
    useEffect(() => { setLocalTheme(isDarkFromContext ? "dark" : "light"); }, [isDarkFromContext]);
    function handleToggleTheme() { if (toggleTheme)
    ...
    function openModalForDate(dateObj, suggestedStartTime = null, durationMin = 60) {
        ...
        if (isBefore(startOfDay(dateObj), todayStart))
            return;
        const key = format(dateObj, "yyyy-MM-dd");
        let start = "09:00";
        let end = "10:00";
    ...
    const containerBg = theme === "dark" ? "bg-gradient-to-br from-slate-900 to-slate-800" : "bg-gradient-to-br from-white via-slate-50 to-indigo-50";
    const headerH = "12rem";
    return (_jsxs("div", { className: `min-h-screen transition-colors duration-300 ${containerBg}`, children: [_jsxs("div", { className: "mx-auto max-w-7xl px-4 sm:px-6 lg:px-8", children: [_jsx("div", { className: "sticky top-0 z-20", children: _jsxs("div", { className: "py-8", children: [_jsxs("div", { className: "mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4", children: [_jsxs("div", { children: [_jsx("h1", { className: `text-3xl font-semibold tracking-tight ${theme === "dark" ? "text-white" : "text-slate-900"}`, children: "Calendar" }), _jsx("p", { className: `text-sm ${theme === "dark" ? "text-slate-300" : "text-slate-600"}`, children: "Professional schedule with light & dark themes \u2014 click a day or a time slot to create an event." })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsxs(Button, { variant: "outline", onClick: today, className: "gap-2", children: [_jsx(CalendarIcon, { className: "h-4 w-4" }), " Today"] }), _jsxs(Button, { onClick: () => { resetForm(); setOpenModal(true); }, className: "flex items-center gap-2", children: [_jsx(Plus, { className: "h-4 w-4" }), " Create"] }), _jsxs("button", { onClick: handleToggleTheme, "aria-label": "Toggle theme", className: `ml-2 inline-flex items-center gap-2 rounded-full px-3 py-2 ring-1 transition-colors ${theme === "dark" ? "bg-slate-800 ring-slate-700 text-slate-100 hover:bg-slate-700" : "bg-white ring-slate-200 text-slate-800 hover:bg-slate-50"}`, children: [theme === "dark" ? _jsx(Moon, { className: "h-4 w-4" }) : _jsx(Sun, { className: "h-4 w-4" }), _jsx("span", { className: "text-sm font-medium", children: theme === "dark" ? "Dark" : "Light" })] })] })] }), _jsx(Card, { className: `mb-6 ${theme === "dark" ? "bg-slate-800 border-slate-700" : "bg-white"}`, children: _jsxs(CardContent, { className: `flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 py-3 ${theme === "dark" ? "text-slate-200" : "text-slate-700"}`, children: [_jsxs("div", { className: "flex items-center gap-3", children: [_jsx(Button, { variant: "ghost", size: "icon", onClick: prev, children: _jsx(ChevronLeft, { className: "h-5 w-5" }) }), _jsx("div", { className: "min-w-[14rem] text-center text-lg font-medium tracking-tight", children: format(currentDate, "MMMM yyyy") }), _jsx(Button, { variant: "ghost", size: "icon", onClick: next, children: _jsx(ChevronRight, { className: "h-5 w-5" }) })] }), _jsxs("div", { className: "flex items-center gap-2", children: [_jsx(Tabs, { value: selectedView, children: _jsxs(TabsList, { className: "grid grid-cols-3", children: [_jsx(TabsTrigger, { className: "px-4", value: "Month", onClick: () => setSelectedView("Month"), children: "Month" }), _jsx(TabsTrigger, { className: "px-4", value: "Week", onClick: () => setSelectedView("Week"), children: "Week" }), _jsx(TabsTrigger, { className: "px-4", value: "Day", onClick: () => setSelectedView("Day"), children: "Day" })] }) }), _jsxs(Button, { variant: "outline", size: "sm", className: "flex items-center gap-2", children: [_jsx(Grid, { className: "h-4 w-4" }), " ", selectedView] })] })] }) })] }) }), _jsx("div", { className: "grid grid-cols-1 gap-6 no-scrollbar", style: { maxHeight: `calc(100vh - ${headerH})`, overflowY: "auto", overflowX: "hidden" }, children: _jsx("main", { children: _jsxs(Card, { className: `${theme === "dark" ? "bg-slate-800 border-slate-700" : "bg-white"} overflow-hidden`, children: [_jsx(CardHeader, { children: _jsx(CardTitle, { className: `text-sm ${theme === "dark" ? "text-slate-200" : "text-slate-700"}`, children: selectedView === "Month" ? "Month view" : selectedView === "Week" ? "Week view" : "Day view" }) }), _jsx(CardContent, { className: "p-4", children: selectedView === "Month" ? (_jsxs("div", { className: "grid grid-cols-7 gap-3", children: [['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'].map(d => (_jsx("div", { className: `text-xs ${theme === "dark" ? "text-slate-300" : "text-muted-foreground"} p-2 text-center font-semibold`, children: d }, d))), monthGrid.map((day, idx) => {
                                                    const key = format(day, "yyyy-MM-dd");
                                                    const inMonth = isSameMonth(day, currentDate);
                                                    const isToday = isSameDay(day, new Date());
                                                    const evs = eventsByDate.get(key) || [];
                                                    ...
                                                    return (_jsxs("button", { onClick: () => {
                                                        ...
                                                        }, "aria-disabled": isPast, disabled: isPast, className: `relative group p-3 text-left min-h-[96px] rounded-lg transition-shadow hover:shadow-lg ${inMonth ? (theme === "dark" ? 'bg-slate-700' : 'bg-white') : (theme === "dark" ? 'bg-slate-800 text-slate-500' : 'bg-slate-50 text-slate-400')} ${isPast ? 'opacity-50 cursor-not-allowed' : ''}`, children: [_jsxs("div", { className: "flex items-start justify-between", children: [_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("div", { className: `w-8 h-8 flex items-center justify-center rounded-full text-sm font-semibold ${isToday ? 'bg-indigo-600 text-white' : (theme === 'dark' ? 'bg-slate-600 text-slate-100' : '')}`, children: format(day, 'd') }), _jsx("div", { className: `text-xs ${theme === 'dark' ? 'text-slate-300' : 'text-muted-foreground'} hidden sm:block`, children: format(day, 'MMM') })] }), _jsx("div", { className: "text-xs text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity", children: "Open" })] }), _jsxs("div", { className: "mt-3 space-y-1", children: [evs.slice(0, 2).map(ev => (_jsxs("div", { className: "flex items-center gap-2", children: [_jsx("span", { className: `inline-block w-2 h-2 rounded-full ${ev.color}` }), _jsx("div", { className: "text-sm truncate", children: ev.title })] }, ev.id))), evs.length > 2 && _jsxs("div", { className: "text-xs text-muted-foreground", children: ["+", evs.length - 2, " more"] })] })] }, idx));
                                                })] })) : selectedView === "Week" ? (
                                        /* WEEK */
                                        _jsx("div", { className: `border rounded-lg overflow-hidden ${theme === 'dark' ? 'border-slate-700' : ''}`, children: _jsxs("div", { className: "w-full", style: {
                                                    display: 'grid',
                                                    gridTemplateColumns: `80px repeat(7, 1fr)`,
                                                    gridTemplateRows: `auto ${DAY_SLOTS.map(() => `${SLOT_HEIGHT_PX}px`).join(' ')}`,
                                                }, children: [_jsx("div", { style: { gridColumn: 1, gridRow: 1 }, className: `px-3 py-3 text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'}`, children: "Time" }), Array.from({ length: 7 }).map((_, i) => {
                                                        const day = addDays(startOfWeek(currentDate, { weekStartsOn: 1 }), i);
                                                        return (_jsxs("div", { style: { gridColumn: i + 2, gridRow: 1 }, className: `px-3 py-2 text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`, children: [_jsx("div", { className: `text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'}`, children: format(day, 'EEE') }), _jsx("div", { className: "text-sm", children: format(day, 'dd MMM') })] }, i));
                                                    }), DAY_SLOTS.map((t, rowIndex) => {
                                                        const row = rowIndex + 2;
                                                        return (_jsxs(React.Fragment, { children: [_jsx("div", { style: { gridColumn: 1, gridRow: row }, className: `px-2 text-[12px] flex items-center ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'}`, children: t.endsWith(':00') ? t.split(':')[0] + ':00' : '' }), Array.from({ length: 7 }).map((_, dayIdx) => {
                                                                    const day = addDays(startOfWeek(currentDate, { weekStartsOn: 1 }), dayIdx);
                                                                    const dayKey = format(day, 'yyyy-MM-dd');
                                                                    const evs = eventsByDate.get(dayKey) || [];
                                                                    const slotEvents = evs.filter((ev) => {
                                                                    ...
                                                                    return (_jsx("div", { style: { gridColumn: dayIdx + 2, gridRow: row }, className: "p-1", children: _jsxs("button", { type: "button", onClick: () => {
                                                                                if (isPastDay)
                                                                                    return;
                                                                                setSelectedSlot({ date: day, start: t });
                                                                                setOpenSlotModal(true);
                                                                            }, "aria-disabled": isPastDay, disabled: isPastDay, className: `w-full h-full rounded-lg transition-shadow flex items-center justify-between px-3 ${isSelected ? (theme === 'dark' ? "bg-indigo-900/30 ring-1 ring-indigo-400/30" : "bg-indigo-50 ring-1 ring-indigo-200") : (theme === 'dark' ? "bg-transparent hover:bg-white/6" : "bg-gray-50 hover:bg-gray-100")}` + (isPastDay ? ' opacity-50 cursor-not-allowed' : ''), style: theme === 'dark' ? { background: 'rgba(255,255,255,0.06)', height: '100%' } : { height: '100%' }, children: [_jsx("div", { className: "text-sm truncate", children: slotEvents[0] ? slotEvents[0].title : "" }), _jsx("div", { className: "flex items-center gap-2", children: slotEvents.length > 0 && _jsx("div", { className: `w-3 h-3 rounded-full ${slotEvents[0].color}` }) })] }) }, dayIdx));
                                                                })] }, t));
                                                    })] }) })) : (
                                        /* DAY view */
                                        _jsxs("div", { className: `border rounded-lg overflow-hidden ${theme === 'dark' ? 'border-slate-700' : ''}`, children: [_jsxs("div", { className: `flex border-b ${theme === 'dark' ? 'bg-slate-900' : 'bg-slate-50'}`, children: [_jsx("div", { className: `w-20 border-r px-3 py-3 text-xs ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'}`, children: "Time" }), _jsx("div", { className: `flex-1 px-4 py-3 text-sm font-medium ${theme === 'dark' ? 'text-slate-200' : ''}`, children: format(currentDate, 'EEEE, dd MMM yyyy') })] }), _jsxs("div", { className: "flex", children: [_jsx("div", { className: `w-20 border-r ${theme === 'dark' ? 'bg-slate-900' : 'bg-white'}`, children: DAY_SLOTS.map((t) => (_jsx("div", { className: `px-2 text-[12px] ${theme === 'dark' ? 'text-slate-400' : 'text-muted-foreground'} border-b flex items-center`, style: { height: SLOT_HEIGHT_PX }, children: t.endsWith(':00') ? t.split(':')[0] + ':00' : '' }, t))) }), _jsx("div", { className: "flex-1 p-4", children: _jsx("div", { className: "space-y-3", children: DAY_SLOTS.map((t, slotIdx) => {
                                                                    const isSelected = selectedSlot && selectedSlot.date && format(selectedSlot.date, 'yyyy-MM-dd') === format(currentDate, 'yyyy-MM-dd') && selectedSlot.start === t;
                                                                    const evs = eventsByDate.get(format(currentDate, 'yyyy-MM-dd')) || [];
                                                                    const slotEvents = evs.filter((ev) => {
                                                                        const s = timeToMinutes(ev.start);
                                                                    ...
                                                                    return (_jsx("div", { style: { height: SLOT_HEIGHT_PX }, children: _jsxs("button", { type: "button", onClick: () => {
                                                                                if (isPastDay)
                                                                                    return;
                                                                                setSelectedSlot({ date: currentDate, start: t });
                                                                                setOpenSlotModal(true);
                                                                            }, "aria-disabled": isPastDay, disabled: isPastDay, className: `w-full h-full rounded-lg transition-shadow flex items-center justify-between px-4 ${isSelected ? (theme === 'dark' ? "bg-indigo-900/30 ring-1 ring-indigo-400/30" : "bg-indigo-50 ring-1 ring-indigo-200") : (theme === 'dark' ? "bg-transparent hover:bg-white/3" : "bg-gray-50 hover:bg-gray-100")}` + (isPastDay ? ' opacity-50 cursor-not-allowed' : ''), children: [_jsx("div", { className: "text-sm truncate", children: slotEvents[0] ? slotEvents[0].title : "" }), _jsx("div", { children: slotEvents.length > 0 && _jsx("div", { className: `w-3 h-3 rounded-full ${slotEvents[0].color}` }) })] }) }, t + slotIdx));
                                                                }) }) })] })] })) })] }) }) })] }), _jsx(SlotModal, { open: openSlotModal, onClose: () => setOpenSlotModal(false), slot: selectedSlot, theme: theme, events: selectedSlot
                    ? (eventsByDate.get(format(selectedSlot.date, "yyyy-MM-dd")) || []).filter((ev) => {
                        if (!ev?.start || !ev?.end)
                            return false;
                        const s = timeToMinutes(ev.start);
                    ...
                    : [], onCreate: (date, start) => openModalForDate(date, start, 60) }), _jsx(EventModal, { open: openModal, onClose: () => setOpenModal(false), initialValues: initialValues, onSave: handleSave, isDark: theme === "dark" }), _jsx("style", { children: `
        .no-scrollbar::-webkit-scrollbar { display: none; }
        .no-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
        .bg-indigo-600 { background-color: #4f46e5; }
        .bg-emerald-500 { background-color: #10b981; }
